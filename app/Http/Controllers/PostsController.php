...
class PostsController extends Controller
{
    ...
    public function savePost(SavePostRequest $request)
    {
        try {
            ...
            if ($postID) {
                ...
                if ($type == "create") {
                    $this->tweetNewPost($postID, $attachments, Auth::user());
                }
            }
    ...
    private function tweetNewPost($postID, $attachmentIds, $user)
    {
        
        $post = Post::where('id', $postID)->where('user_id', $user->id)->first();
        $text = $post->text;
        Log::info("i get a call ".$postID);

        // Get the file paths of the attachments
        $mediaFiles = Attachment::whereIn('id', $attachmentIds)
            ->pluck('filename')
        ...
        // Tweet with media
        $result = $this->twitterService->tweetWithMedia($user, $text, $mediaFiles);

        if (!$result['success']) {
            Log::error("Failed to tweet new post. Post ID: {$post->id}. Error: " . json_encode($result['error']));
        }
    }
